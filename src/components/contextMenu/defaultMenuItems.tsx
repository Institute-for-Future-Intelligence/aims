...
export const EnergyGraphCheckBox = () => {
  const visible = useStore(Selector.energyGraphVisible);
  const setChanged = usePrimitiveStore(Selector.setChanged);
  const { t } = useTranslation();
  const lang = useLanguage();
  ...
  const setVisible = (checked: boolean) => {
    useStore.getState().set((state) => {
      state.projectState.energyGraphVisible = checked;
    });
    setChanged(true);
  };
  ...
  return (
    <MenuItem stayAfterClick={false} hasPadding={false}>
      ...
      >
        {t('molecularViewer.EnergyGraph', lang)}
      </Checkbox>
    </MenuItem>
  );
};
